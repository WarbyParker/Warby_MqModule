...
class PhpAmqpLib_Wire_AMQPReader
{
    ...
    public function __construct($str, PhpAmqpLib_Wire_IO_AbstractIO $io = null, $timeout = 0)
    {
        if (!function_exists("bcmul")) {
            throw new PhpAmqpLib_Exception_AMQPRuntimeException("'bc math' module required");
        }
     ...
     *
     * @throws \PhpAmqpLib\Exception\PhpAmqpLib_Exception_AMQPTimeoutException
     */
    protected function wait()
    {
        ...
        if ($result === false) {
            throw new PhpAmqpLib_Exception_AMQPRuntimeException(sprintf("An error occurs", $this->timeout));
        }

        if ($result === 0) {
            throw new PhpAmqpLib_Exception_AMQPTimeoutException(sprintf("A timeout occurs while waiting for incoming data", $this->timeout));
        }
    }
     ...
     * @throws \PhpAmqpLib\Exception\PhpAmqpLib_Exception_AMQPRuntimeException
     */
    ...
    protected function rawread($n)
    {
        if ($this->io) {
            ...
            if (strlen($this->str) < $n) {
                throw new PhpAmqpLib_Exception_AMQPRuntimeException("Error reading data. Requested $n bytes while string buffer has only " .
                    strlen($this->str));
            }
